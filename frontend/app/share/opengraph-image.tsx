...
export default async function Image({
  ...
  try {
    const fetchPart = async (id: string): Promise<Part | null> => {
      try {
    ...
    const results = await Promise.all([
      cpuId ? fetchPart(cpuId) : null,
      gpuId ? fetchPart(gpuId) : null,
      memoryId ? fetchPart(memoryId) : null,
      storage1Id ? fetchPart(storage1Id) : null,
      osId ? fetchPart(osId) : null,
    ])
