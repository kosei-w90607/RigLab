...
| `CORS_ORIGINS` | (本番フロントエンドURL) | 例: `https://your-domain.com` |
...
| `FRONTEND_URL` | (本番フロントエンドURL) | メール内リンク生成用（例: `https://your-domain.com`） |
...
| `RAKUTEN_ALLOWED_WEBSITE` | No | 楽天API許可ドメイン | デフォルト: `https://your-domain.com` |
| `RESEND_API_KEY` | Yes | Resendメール送信API キー | `re_xxxxxxxxxx` |
| `FRONTEND_URL` | Yes | フロントエンドURL（メール内リンク生成用） | `https://your-domain.com` |
