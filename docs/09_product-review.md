...
| API | 用途 | 期待効果 |
|-----|------|----------|
| Amazon PA-API | パーツ情報・価格の自動取得 | 手入力の工数削減、リアルタイム価格 |
| 楽天商品検索API | 価格比較データの取得 | 複数ECサイト横断の価格比較 |
| （独自集計） | 価格推移グラフ | 買い時判断の支援 |

**実装候補機能:**
- パーツ情報の自動取得・更新（管理者の手入力工数を大幅削減）
